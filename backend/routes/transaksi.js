        ...
        .then((result) => {
          ...
          if (req.body.status === "lunas") {
            meja.update(
              { status: "kosong" },
              { where: { id_meja: req.body.id_meja } }
            ); // mengubah status meja menjadi kosong
          }
        })
        .catch((error) => {
          // jika gagal
...
app.put("/bayar/:id_transaksi", async (req, res) => {
  ...
  transaksi
    .update({ status: `lunas` }, { where: param }) // menghapus data transaksi berdasarkan id transaksi yang dikirimkan melalui parameter
    .then(async (result) => {
      /** get data transaksi */
